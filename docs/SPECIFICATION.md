...
| F5 | LINE通知（エントリー） | エントリー処理結果（OK/NG+理由）をLINEで通知 | P1 |
...
| F9 | LINE通知（決定・辞退） | 決定・辞退処理の結果をLINEで通知 | P1 |
...
[MCia] Step 2: AI分類判定（Claude Haiku）
    ...
    │
    ├─ 「決定連絡」 → フロー2（決定・辞退処理）へ
    │    判定基準: 以下のキーワード・意図を含む + 案件特定情報
    │      ・「決定」「採用」「アサイン」
    │      ・「日程確保をお願いいたします」「日程を確保してください」等の日程確保依頼の意図
    │
    ├─ 「辞退受理」 → ログのみ記録
    │    判定基準: 辞退に対する事務所からの返信
    │
...
[MCia] メール本文から案件情報を抽出（Claude Haiku）
    ...
    └─ その他条件
    ↓
[MCia] Googleカレンダーで日程の空き確認（→ 4.5a 識別・重複判定ロジック参照）
    ↓
┌─ 空きあり / MCia仮予定のみと重複 ─────────────┐
│  [MCia] エントリー希望メールの下書きを             │
│         Gmailに自動作成                         │
│  ※ MCia仮予定同士の重複はブロックしない           │
│   （決定時に辞退で解消するワークフロー）           │
│                                                │
│  ┌─ PR要素あり ─────────────────────────────┐ │
│  │ [MCia] 過去の処理済み案件DBから                │ │
...
### 4.5a Googleカレンダー予定の識別・重複判定ロジック

#### 課題

ユーザーのGoogleカレンダーにはMC案件以外にも個人の予定（プライベート、他の仕事、趣味等）が入っている。全てのカレンダー予定を「重複」として扱うと、空白時間にしか案件を入れられないアプリになってしまう。

そのため、カレンダー上の予定が以下のどれに該当するかを識別し、重複判定の対象を適切に絞り込む必要がある。

#### カレンダー予定の分類

| 分類 | 説明 | 重複判定 |
|------|------|---------|
| **A. MCia管理予定** | MCiaが作成・管理している予定（DB上に `calendar_event_id` が存在） | 対象 |
| **B. MC関連予定（MCia導入前）** | ユーザーがMCia導入前に手動で入れたMC案件の仮予定 | 対象（要識別） |
| **C. 個人予定** | プライベートの予定、他業種の仕事など | 対象外 |

#### 識別ロジック（3段階判定）

```
[MCia] カレンダーから該当期間の予定を取得
    ↓
[Step 1] MCia管理予定の照合
    │  Googleカレンダー Event ID を projects テーブルの calendar_event_id と照合
    │  → 一致する場合: 「MCia管理予定（分類A）」→ MC案件として重複判定対象
    │  → 一致しない場合: Step 2へ
    ↓
[Step 2] MC関連予定のヒューリスティック判定
    │  以下の条件で「MC案件の仮予定である可能性」をスコアリング:
    │
    │  ■ タイトル判定（高スコア）
    │    ・事務所名を含む（ユーザー登録済みの agencies テーブルと照合）
    │    ・MC/コンパニオン業界キーワードを含む
    │      例: 「MC」「コンパニオン」「司会」「受付」「ナレーション」
    │          「展示会」「イベント」「発表会」「モーターショー」「説明会」等
    │    ・「仮」「仮押さえ」「仮確保」等の文言を含む
    │
    │  ■ 説明欄判定（中スコア）
    │    ・事務所名、案件名、報酬額等のMC関連情報を含む
    │
    │  ■ 終日予定判定（参考スコア）
    │    ・MC案件は終日予定で登録されることが多い（日単位の仕事）
    │    ・時間指定のある予定はプライベートの可能性が高い
    │
    │  → スコアが閾値以上: 「MC関連予定（分類B）の可能性が高い」
    │  → スコアが閾値未満: Step 3へ
    ↓
[Step 3] 不明な予定の処理
    │  スコアが判定閾値未満の予定
    │  → 「個人予定（分類C）」として扱い、重複判定の対象外とする
    │  → ただし、初回セットアップ時にユーザーに確認UIを提供（後述）
```

#### 初回セットアップ時のカレンダー学習

MCia導入時、ユーザーの既存カレンダー予定を一括スキャンし、未来の予定について分類を提示する。

```
[MCia] 初期設定フロー（S3）にて
    ↓
[MCia] 未来のカレンダー予定を取得（最大3ヶ月先まで）
    ↓
[MCia] ヒューリスティック判定でMC関連の可能性が高い予定をリストアップ
    ↓
[ユーザー] リストを確認し、各予定を「MC案件」or「個人予定」に分類
    │
    ├─ 「MC案件」に分類された予定
    │    → projects テーブルに status='entered' として登録
    │    → calendar_event_id を紐付け
    │    → 以降はMCia管理予定（分類A）として扱われる
    │
    └─ 「個人予定」に分類された予定
         → MCiaの管理対象外（重複判定に使わない）
```
...
[MCia] 新規案件の日程で空き確認
    ↓
[MCia] 該当期間のカレンダー予定を取得
    ↓
[MCia] 各予定を上記の3段階判定で分類
    ↓
┌─ MCia管理予定（分類A）と重複 ────────────────────────────┐
│  ├─ 重複先もMCiaの仮予定 → 「重複あり」だがエントリー可能  │
│  │   （仮予定同士は重複してもエントリーし、決定時に辞退で解消）│
│  │   → エントリー下書きを作成                             │
│  └─ 重複先がMCiaの確定予定 → 「重複あり（確定）」          │
│      → エントリー不可、LINE通知でスキップ理由を通知          │
└──────────────────────────────────────────────────────────┘
...
1. **MCia仮予定同士の重複はブロックしない**: MC業界ではエントリー段階で複数案件に応募し、決定後に他を辞退するのが通常のワークフロー。仮予定同士が重複してもエントリーは許可する
2. **確定予定との重複のみブロック**: MCia管理の「確定」予定と重複する場合のみ、自動的にエントリーをブロックする
3. **個人予定は無視**: MC案件と判別できない予定は重複判定の対象外とし、カレンダーが埋まっていてもエントリー可能とする
4. **判断に迷う場合はユーザーに委ねる**: MC関連予定（分類B）との重複時はユーザーに確認を求め、誤ブロックを防ぐ
...
| バックエンド/ホスティング | Cloudflare Workers (Hono) + Static Assets + TypeScript | フロント・API統合、エッジ実行、型安全なAPI開発 |
...
| 認証 | Auth.js v5 (NextAuth.js v5) + `@auth/d1-adapter` | 無料OSS、公式D1アダプター、Google OAuth対応、Edge互換 |
