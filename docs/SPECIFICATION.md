...
| F13 | 請求書データエクスポート | 月次・事務所別の確定案件一覧をテーブル表示/CSV出力。請求書作成支援 | P1 |
...
### 4.6 請求書データエクスポート機能（F13）

#### 概要
フリーランス（一人親方等）のMC/コンパニオンが各事務所に請求書を送る際に必要な、月次の案件データを事務所別に一覧表示・エクスポートする機能。ユーザーが自分の請求書フォーマットに項目をコピペしやすい形式で提供する。

#### 対象データ
- `projects` テーブルの `status = 'confirmed'` の案件
- `agency_id` で事務所別にフィルタ
- `start_date` で年月フィルタ
...
|------|----------|------|
| 案件名 | `title` | イベント名・案件名 |
| 担当日付 | `start_date` 〜 `end_date` | 実施日（複数日の場合は日数も表示） |
| 金額 | `compensation` | 報酬額 |
| 場所 | `location` | 会場名（補助情報） |

#### 提供方式（3段階）

**1. アプリ内テーブル表示 + コピーボタン（P0）**
```
┌──────────────────────────────────────────────────┐
│  請求データ   2026年2月  ○○事務所  [▼月選択]      │
├──────────────────────────────────────────────────┤
│  案件名              │ 日付        │ 金額         │
│──────────────────────┼────────────┼─────────────│
│  東京モーターショー MC │ 2/5        │ ¥30,000     │
│  新商品発表会 司会    │ 2/12〜2/13 │ ¥50,000     │
│  合同企業説明会 受付  │ 2/20       │ ¥15,000     │
│──────────────────────┼────────────┼─────────────│
│  合計               │ 4日間       │ ¥95,000     │
├──────────────────────────────────────────────────┤
│  [📋 テーブルをコピー]  [📥 CSVダウンロード]        │
...
- 「テーブルをコピー」ボタン: タブ区切りテキストをクリップボードにコピー
  → Excel / Googleスプレッドシートにペーストするとセル構造が維持される
- HTMLテーブルの選択コピーでもセル構造が維持される

**2. CSVダウンロード（P0）**
- BOM付きUTF-8（Excelで文字化けしない）
- ファイル名: `MCia_請求データ_○○事務所_2026年02月.csv`

```csv
案件名,担当日付,日数,金額
東京モーターショー MC,2026-02-05,1,30000
新商品発表会 司会,2026-02-12〜2026-02-13,2,50000
合同企業説明会 受付,2026-02-20,1,15000
,,合計,95000
```

**3. Excelダウンロード（P1）**
- `.xlsx` 形式（SheetJSライブラリ使用）
- セル書式設定済み（金額列は数値フォーマット、ヘッダー太字等）

#### 画面配置
- ダッシュボード（S4）のタブまたはサイドメニューからアクセス
- 専用画面 `/invoice-data` を新設
...
| バックエンド/ホスティング | Cloudflare Workers (Hono) + Static Assets | フロント・API統合、エッジ実行 |
| Next.js アダプター | @opennextjs/cloudflare | Workers上でNext.jsをフルサポート |
...
### 5.2 Cloudflare Workers統合アーキテクチャ

**Cloudflare Pagesは2025年4月に廃止（メンテナンスモード）** となったため、全てをCloudflare Workersに統合する。

**フロントエンド + バックエンド = 1つのWorker:**
- Next.js は `@opennextjs/cloudflare` (OpenNext adapter) を使用してWorkers上で動作
- Static Assets機能でフロントエンドの静的ファイルを配信（リクエスト無料）
- APIルートとSSRはWorkerスクリプトとして実行
- `wrangler.jsonc` でassets directory, D1, KV, Queuesのバインディングを一元管理

**構成ファイル (wrangler.jsonc):**
```jsonc
{
  "name": "mcia",
  "compatibility_date": "2025-04-01",
  "assets": { "directory": ".open-next/assets", "binding": "ASSETS" },
  "d1_databases": [{ "binding": "DB", "database_name": "mcia-db" }],
  "kv_namespaces": [{ "binding": "KV", "id": "..." }],
  "queues": {
    "producers": [{ "binding": "EMAIL_QUEUE", "queue": "email-processing" }],
    "consumers": [{ "queue": "email-processing", "max_batch_size": 10 }]
  }
}
```

| 観点 | Cloudflare Workers | 備考 |
|------|-------------------|------|
| コスト | Paid: $5/月で1,000万リクエスト、Static Assets無料 | 1,000ユーザーに十分 |
...
| 統合性 | フロント+API+DB全て1つのWorkerで管理 | デプロイ・運用がシンプル |
...
│                                                  │
│  ┌──────────────────────────────────────────────┐│
│  │     Cloudflare Workers (統合: Frontend+API)  ││
│  │     Next.js (@opennextjs/cloudflare)         ││
│  │     + Static Assets (HTML/CSS/JS配信)        ││
│  │                                              ││
│  │  ┌─────────────┐  ┌────────────────────┐    ││
│  │  │ Webhook     │  │ REST API           │    ││
│  │  │ Handlers    │  │ Endpoints          │    ││
...
| Method | Endpoint | 説明 |
|--------|----------|------|
| GET | `/api/invoice-data?agency_id={id}&year={y}&month={m}` | 請求データ取得（JSON） |
| GET | `/api/invoice-data?agency_id={id}&year={y}&month={m}&format=csv` | 請求データCSVダウンロード |
...
- 事務所側の管理画面
- 複数MC間の日程調整機能
- 請求書PDF自動生成（F13のデータを元にテンプレートから生成）
- Googleスプレッドシート連携（月次データの自動書き出し）
...
| S10 | 請求データ | `/invoice-data` | 月次・事務所別の確定案件一覧・エクスポート |
